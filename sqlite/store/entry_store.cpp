...
bool entry_store::logical_delete(int id) const {
  ...
  if (!dao::detail::load_entry(db, id, dummy, nullptr, false)) return false;
...
bool entry_store::update_text(int id, const std::string& content) const {
  ...
  if (!dao::detail::load_entry(db, id, current)) return false;
  if (!dao::detail::exec_simple(db, "BEGIN IMMEDIATE;")) return false;
...
std::optional<std::pair<std::vector<karing::dao::KaringRecord>, int>> entry_store::move_entry_before(int id, int before_id) const {
  if (id == before_id) return std::nullopt;
  ...
  auto store_state = [&](int slot_id, const entry_state& state) -> bool {
    sqlite3_stmt* stmt = nullptr;
    const char* sql =
        "UPDATE entries SET "
        "used=?, source_kind=?, media_kind=?, content_text=?, file_path=?, original_filename=?, "
    ...
    bind_optional_int64(stmt, 9, state.stored_at);
    bind_optional_int64(stmt, 10, state.updated_at);
    sqlite3_bind_int(stmt, 11, slot_id);
    const bool ok = sqlite3_step(stmt) == SQLITE_DONE;
    sqlite3_finalize(stmt);
    return ok;
  };
  ...
  if (!states.has_value() ||
      ...
      before_id > static_cast<int>(states->size()) ||
      !(*states)[static_cast<size_t>(id - 1)].used ||
      !(*states)[static_cast<size_t>(before_id - 1)].used) {
    dao::detail::exec_simple(db, "ROLLBACK;");
    return std::nullopt;
  }
  ...
  const auto moving_it = std::find_if(active_states.begin(), active_states.end(), [&](const auto& item) {
    return item.first == id;
  });
  const auto before_it = std::find_if(active_states.begin(), active_states.end(), [&](const auto& item) {
    return item.first == before_id;
  });
  if (moving_it == active_states.end() || before_it == active_states.end()) {
    dao::detail::exec_simple(db, "ROLLBACK;");
    return std::nullopt;
  }

  auto moving = std::move(*moving_it);
  active_states.erase(moving_it);
  const auto insert_it = std::find_if(active_states.begin(), active_states.end(), [&](const auto& item) {
    return item.first == before_id;
  });
  active_states.insert(insert_it, std::move(moving));

  std::vector<entry_state> moved(states->size());
  for (size_t i = 0; i < active_states.size(); ++i) {
    moved[i] = active_states[i].second;
  }

  for (size_t i = 0; i < moved.size(); ++i) {
    if (!store_state(static_cast<int>(i + 1), moved[i])) {
      dao::detail::exec_simple(db, "ROLLBACK;");
      return std::nullopt;
    }
  }

  const int next_id = active_states.size() >= moved.size() ? 1 : static_cast<int>(active_states.size()) + 1;
  ...
  if (!next_ok ||
      !dao::detail::exec_simple(db, "INSERT INTO entries_fts(entries_fts) VALUES('rebuild');") ||
  ...
  for (size_t i = 0; i < active_states.size(); ++i) {
    ...
    if (!dao::detail::load_entry(db, static_cast<int>(i + 1), record, &ignored_path)) return std::nullopt;
