...
int run_add(const std::string& base_url,
  ...
  if (file_path.has_value()) {
    ...
    const auto response = http::multipart_form(base_url, "/", "POST", {}, "file", *file_path, name, mime, api_key);
  ...
  writer["indentation"] = "";
  const auto response = http::post_json(base_url, "/", Json::writeString(writer, body), api_key);
  return json_output ? utils::print_response_json(response) : utils::print_response(response, false);
}
