...
static std::vector<std::string> split_terms(const std::string& raw) {
...
QueryBuild build_live_fts_query(const std::string& raw) {
  ...
  if (s.size() > 512) s = s.substr(0, 512);

  const auto terms = split_terms(s);
  if (terms.empty()) {
    qb.err = "empty";
    return qb;
  }
